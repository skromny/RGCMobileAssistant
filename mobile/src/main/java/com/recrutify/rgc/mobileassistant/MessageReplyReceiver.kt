...
class MessageReplyReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (CallMessagingService.REPLY_ACTION == intent.action) {
            val conversationId = intent.getIntExtra(CallMessagingService.CONVERSATION_ID, -1)
    ...
    private fun getMessageText(intent: Intent): CharSequence? {
        ...
        return remoteInput?.getCharSequence(CallMessagingService.EXTRA_VOICE_REPLY)
    }
